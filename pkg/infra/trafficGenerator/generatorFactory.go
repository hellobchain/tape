...
func (t *TrafficGenerator) CreateGeneratorWorkers(mode int) ([]infra.Worker, error) {
	generatorWorkers := make([]infra.Worker, 0)
	...
	if mode/infra.PROPOSALFILTER == 1 {
		...
		if err != nil {
			return generatorWorkers, err
		}
		generatorWorkers = append(generatorWorkers, proposers)
		...
		for i := 0; i < t.signerNumber; i++ {
			generatorWorkers = append(generatorWorkers, assembler)
			generatorWorkers = append(generatorWorkers, Integrator)
		}
	}
	// if broadcaster int mod 3 = 0
	if mode%infra.COMMITFILTER == 0 {
		broadcaster, err := CreateBroadcasters(t.ctx, t.envs, t.errorCh, t.config, t.logger)
		if err != nil {
			return generatorWorkers, err
		}
		generatorWorkers = append(generatorWorkers, broadcaster)
	}
	...
	for i := 0; i < t.parallel; i++ {
		if mode%infra.QUERYFILTER == 0 {
			...
			generatorWorkers = append(generatorWorkers, Initiator)
		} else {
			// if fake int mod 2 = 1
			...
			generatorWorkers = append(generatorWorkers, fackEnvelopGenerator)
		}
	}
	return generatorWorkers, nil
}
