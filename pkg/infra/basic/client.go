...
import (
	...
	"github.com/wsw365904/tape/internal/fabric/core/comm"
...
func CreateEndorserClient(node Node, logger *wlogging.WswLogger) (peer.EndorserClient, error) {
...
func CreateBroadcastClient(ctx context.Context, node Node, logger *wlogging.WswLogger) (orderer.AtomicBroadcast_BroadcastClient, error) {
	conn, err := DialConnection(node, logger)
	if err != nil {
		return nil, err
	}
...
func CreateDeliverFilteredClient(ctx context.Context, node Node, logger *wlogging.WswLogger) (peer.Deliver_DeliverFilteredClient, error) {
	conn, err := DialConnection(node, logger)
	if err != nil {
		return nil, err
	}
...
func DialConnection(node Node, logger *wlogging.WswLogger) (*grpc.ClientConn, error) {
